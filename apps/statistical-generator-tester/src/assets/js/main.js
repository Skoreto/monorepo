...
function generateNumbers() {
  ...
  var countNumbersGenerated = parseInt(
    document.getElementById('inpNumbersGenerated').value
  );
  var seedNumber;
...
function testExtremal() {
  ...
  for (i = 0; i < n - 2; i++) {
    if (
      (listNumbers[i] < listNumbers[i + 1] &&
        listNumbers[i + 1] > listNumbers[i + 2]) ||
      (listNumbers[i] > listNumbers[i + 1] &&
        listNumbers[i + 1] < listNumbers[i + 2])
    ) {
      extremalP += 1;
  ...
  if (resultRejected) {
    ...
    $('#extremalBox').append(
      Math.abs(extremalU).toFixed(4) + ' $>$ ' + u0975.toFixed(2) + '<br/>'
    );
    $('#extremalBox').append(
      "<span class='text-red'><b>Zamítá</b></span> se hypotéza o náhodnosti v uspořádání řady hodnot."
    );
  } else {
    $('#extremalBox').append('$|u| \\leq u_{1-{\\alpha / 2}}$<br/>');
    $('#extremalBox').append(
      Math.abs(extremalU).toFixed(4) + ' $\\leq$ ' + u0975.toFixed(2) + '<br/>'
    );
    $('#extremalBox').append(
      "<span class='text-green'><b>Nezamítá</b></span> se hypotéza o náhodnosti v uspořádání řady hodnot."
    );
  }
...
function testDifference() {
  ...
  if (resultRejected) {
    ...
    $('#diffBox').append(
      Math.abs(diffU).toFixed(4) + ' $>$ ' + u0975.toFixed(2) + '<br/>'
    );
    $('#diffBox').append(
      "<span class='text-red'><b>Zamítá</b></span> se hypotéza o náhodnosti v uspořádání řady hodnot."
    );
    ...
    $('#diffBox').append(
      Math.abs(diffU).toFixed(4) + ' $\\leq$ ' + u0975.toFixed(2) + '<br/>'
    );
    $('#diffBox').append(
      "<span class='text-green'><b>Nezamítá</b></span> se hypotéza o náhodnosti v uspořádání řady hodnot."
    );
...
function testSpearman() {
  ...
  $('#spearmanBox').append(
    "<table class='table table-striped table-condensed' id='spearmanTable'>" +
  ...
  $('#spearmanResultsBox').append(
    '$D(r_s)$ = ' + spearmanD.toFixed(4) + '<br/>'
  );
  $('#spearmanResultsBox').append('$r_s$ = ' + spearmanR.toFixed(4) + '<br/>');
  $('#spearmanResultsBox').append(
    '$u$ = ' + spearmanU.toFixed(4) + '<br/><br/>'
  );
  ...
  if (resultRejected) {
    ...
    $('#spearmanResultsBox').append(
      Math.abs(spearmanU).toFixed(4) + ' $>$ ' + u0975.toFixed(2) + '<br/>'
    );
    $('#spearmanResultsBox').append(
      "<span class='text-red'><b>Zamítá</b></span> se hypotéza o nulovém " +
        'koeficientu korelace nezávisloti na pořadí.<br/><br/>'
    );
  } else {
    $('#spearmanResultsBox').append('$|u| \\leq u_{1-{\\alpha / 2}}$<br/>');
    $('#spearmanResultsBox').append(
      Math.abs(spearmanU).toFixed(4) + ' $\\leq$ ' + u0975.toFixed(2) + '<br/>'
    );
    $('#spearmanResultsBox').append(
      "<span class='text-green'><b>Nezamítá</b></span> se hypotéza o nulovém " +
        'koeficientu korelace nezávisloti na pořadí.<br/><br/>'
    );
  }
...
function testSerial() {
  ...
  // SERIALNI KORELACE
  $('#serialBox').append(
    "<table class='table table-striped table-condensed' id='serialTable'>" +
  ...
  for (k = 1; k <= 10; k++) {
    ...
    for (i = 1; i <= n - k; i++) {
      ...
      pom1 +=
        (listNumbers[i - 1] - (1 / (n - k)) * sumXj1) *
        (listNumbers[i + k - 1] - (1 / (n - k)) * sumXj2);
    }
    var rkCov = pom1 / (n - k);
    ...
    if (rkU > u0975) {
      resultText =
        "<span class='text-red'><b>Zamítá</b></span> se hypotéza o nulovém koef. seriální korelace.";
    } else {
      resultText =
        "<span class='text-green'><b>Nezamítá</b></span> se hypotéza o nulovém koef. seriální korelace.";
    }
    $('#serialTable').append(
      '<tr><td>' +
        k +
  ...
  $('#serialBox').append(
    '$H_0: \\rho_1 = \\rho_2 = \\ldots = \\rho_k = 0$ pro $i = 1, 2, \\ldots , k$<br/>'
  );
  $('#serialBox').append('$H_1: \\rho_i \\neq 0$ pro al. jedno $i$ <br/><br/>');
  $('#serialBox').append(
    '$Q = \\sum_{i=1}^k r_i^2 \\doteq ' +
      n +
      '\\cdot' +
      cochSumri2.toFixed(6) +
      ' \\doteq ' +
      cochQ.toFixed(3) +
      '$<br/>'
  );
  $('#serialBox').append(
    '$Q_{1-{\\alpha / 2}, k} = Q_{0.975, 10} = ' + Q0975_10 + '$<br/><br/>'
  );
  ...
  if (resultRejected) {
    ...
    $('#serialBox').append(
      cochQ.toFixed(3) + ' $>$ ' + Q0975_10 + '<br/><br/>'
    );
    $('#serialBox').append(
      "<span class='text-red'><b>Zamítá</b></span> se sdružená hypotéza.<br/><br/>"
    );
  } else {
    $('#serialBox').append('$Q \\leq Q_{1-{\\alpha / 2}, k}$<br/>');
    $('#serialBox').append(
      cochQ.toFixed(3) + ' $\\leq$ ' + Q0975_10 + '<br/><br/>'
    );
    $('#serialBox').append(
      "<span class='text-green'><b>Nezamítá</b></span> se sdružená hypotéza.<br/><br/>"
    );
  }
